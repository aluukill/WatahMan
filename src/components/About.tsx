...
export default function About() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          if (separatorRef.current) {
            const scale = Math.min(1, self.progress * 3)
            separatorRef.current.style.transform = `scaleX(${scale})`
            separatorRef.current.style.opacity = String(scale)
          }
        },
      })
      ...
      tl.fromTo(
        panelRef.current,
        { opacity: 0, y: 80, scale: 0.92, filter: 'blur(20px)' },
      ...
      if (lines && lines.length > 0) {
        tl.fromTo(
          lines,
          { y: 60, opacity: 0, rotateX: -15, filter: 'blur(5px)' },
          { y: 0, opacity: 1, rotateX: 0, filter: 'blur(0px)', duration: 0.8, stagger: 0.12, ease: 'power2.out' },
          '-=0.6'
        )
      }
  ...
  return (
    ...
    >
      <div
        ref={separatorRef}
        className="absolute top-0 left-1/2 -translate-x-1/2 w-1/3 h-px bg-gradient-to-r from-transparent via-accent/30 to-transparent"
      ...
      <div className="content-max-w mx-auto">
        ...
        >
          <span className="text-xs tracking-[0.4em] text-accent/60 uppercase font-medium">
            About the studio
          </span>
        </div>
        ...
        <div
          ref={panelRef}
          className="glass rounded-3xl p-10 md:p-16 lg:p-20 relative overflow-hidden"
          style={{ willChange: 'transform, opacity, filter' }}
        >
          <div className="absolute -top-40 -right-40 w-80 h-80 rounded-full bg-accent/5 blur-[100px]" />
          <div className="absolute -bottom-40 -left-40 w-80 h-80 rounded-full bg-accent-secondary/5 blur-[100px]" />
          ...
          <div className="relative z-10 grid md:grid-cols-2 gap-12 md:gap-20 items-center">
            ...
            <div className="space-y-8">
              <p ref={textRef} className="text-sm md:text-base leading-relaxed text-white/50 font-light">
                We blend cutting-edge technology with cinematic artistry to create
                immersive digital experiences that transcend the ordinary. Every
                project is an opportunity to push the boundaries of what&apos;s
                possible on the web.
              </p>
